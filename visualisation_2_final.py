...
chart2 = alt.Chart(top_names_region).mark_bar().encode(
    x=alt.X('nombre:Q', title='Compte', scale=alt.Scale(domain=[0, max(top_names_france['nombre'].max()/10, top_names_region['nombre'].max())])),
    y=alt.Y('preusuel:N', sort='-x', title='Prénoms'),
    color=alt.Color('sexe:N', scale=alt.Scale(domain=['Homme', 'Femme'], range=['blue', 'pink'])),
    tooltip=['preusuel', 'nombre']
).properties(
...
# Calculate density
total_births_by_region = filtered_data.groupby('region_name')['nombre'].sum().reset_index()
name_births_by_region = filtered_data[filtered_data['preusuel'] == selected_name].groupby('region_name')['nombre'].sum().reset_index()
density_data = pd.merge(name_births_by_region, total_births_by_region, on='region_name', suffixes=('_name', '_total'))
density_data['density'] = density_data['nombre_name'] / density_data['nombre_total']
...
regions_geo = regions_geo.merge(density_data[['region_name', 'density']], on='region_name', how='left')
regions_geo['density'] = regions_geo['density'].fillna(0)  # Fill NaN values with 0

...
st.markdown("<b><small>Sélectionner une période</small></b>", unsafe_allow_html=True)
start_year, end_year = st.select_slider(
    "Year Range Slider", options=year_range, value=(1900, 2020), label_visibility="collapsed"
)

filtered_names = names[(names['annais'] >= start_year) & (names['annais'] <= end_year)]
...
if not filtered_names.empty:
    ...
    aggregated_names = filtered_names.groupby(['preusuel', 'sexe']).agg({'nombre': 'sum'}).reset_index()

    # Map gender to labels
    aggregated_names['gender'] = aggregated_names['sexe'].map({1: 'Male', 2: 'Female'})

    # Filter top 20 male and top 20 female names
    top_20_males = aggregated_names[aggregated_names['sexe'] == 1].nlargest(20, 'nombre')
    top_20_females = aggregated_names[aggregated_names['sexe'] == 2].nlargest(20, 'nombre')

    # Filter least 20 male and least 20 female names
    least_20_males = aggregated_names[aggregated_names['sexe'] == 1].nsmallest(20, 'nombre')
    least_20_females = aggregated_names[aggregated_names['sexe'] == 2].nsmallest(20, 'nombre')
    ...
    top_males_chart = alt.Chart(top_20_males).mark_bar().encode(
        ...
        y=alt.Y('preusuel:N', title='', sort=None, axis=alt.Axis(orient='left', labels=False, ticks=False)),
        color=alt.value('blue'),
        tooltip=[alt.Tooltip('preusuel:N', title='prenom'), alt.Tooltip('nombre:Q', title='nombre'), alt.Tooltip('gender:N', title='sexe')]
    ).properties(
        width=300,
        height=400
    )
    ...
    top_females_chart = alt.Chart(top_20_females).mark_bar().encode(
        x=alt.X('nombre:Q', title='', axis=alt.Axis(labels=True, format='~s')),
        y=alt.Y('preusuel:N', title='', sort=None, axis=alt.Axis(orient='right', labels=False, ticks=False)),
    ...
    # Display the top and least combined charts
    ...
    st.markdown("<b><small>Ajouter ou retirer des prénoms pour la courbe de popularité</small></b>", unsafe_allow_html=True)
    ...
    if st.session_state.selected_names:
        ...
        yearly_counts['rank'] = yearly_counts.groupby('annais')['nombre'].rank(method='first', ascending=False)

        # Get the rank for the selected names
        name_rank_data = yearly_counts[yearly_counts['preusuel'].isin(st.session_state.selected_names)]
        ...
        popularity_chart = alt.Chart(name_rank_data).mark_line(point=alt.OverlayMarkDef(color='orange'), color='orange').encode(
            ...
            y=alt.Y('rank:Q', title='Rang', scale=alt.Scale(domain=(0, name_rank_data['rank'].max() + 1))),
            color='preusuel:N',
            tooltip=[alt.Tooltip('annais:O', title='annee'), alt.Tooltip('rank:Q', title='rang')]
        ).properties(
